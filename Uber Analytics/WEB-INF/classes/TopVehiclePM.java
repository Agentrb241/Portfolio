...
public class TopVehiclePM extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String PathToFirstClass = "webapps/BadAPPles/data/other-Firstclass_B01536.csv";
        String PathToHighClass = "webapps/BadAPPles/data/other-Highclass_B01717.csv";
        String TopVehiclePMCache = "webapps/BadAPPles/Cache/TopVehiclePM.csv";

        File file = new File(TopVehiclePMCache);

        try {
            if(file.exists()){
                BufferedReader br = new BufferedReader(new FileReader(TopVehiclePMCache));
                String line = null;
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    try {
                        ...
                        if (x < 6 && vehicle.contains("FirstClass")) {
                        ...
                        else if (x < 6 && vehicle.contains("HighClass")) {
                ...
                List<String> key2 = firstClass2.entrySet().stream().sorted(Map.Entry.<String, Integer>comparingByValue().reversed()).limit(1).map(Map.Entry::getKey).collect(Collectors.toList());
                List<String> key3 = highClass1.entrySet().stream().sorted(Map.Entry.<String, Integer>comparingByValue().reversed()).limit(1).map(Map.Entry::getKey).collect(Collectors.toList());
                ...
                int maxValueInMap2 = (Collections.max(firstClass2.values()));
                int maxValueInMap3 = (Collections.max(highClass1.values()));
                int maxValueInMap4 = (Collections.max(highClass2.values()));

                String[] stringArray = new String[4];

                stringArray[0] = "FirstClass" + ": " + String.valueOf(maxValueInMap1);
                stringArray[1] = "HighClass" + ": " + String.valueOf(maxValueInMap3);
                stringArray[2] = "FirstClass" + ": " + String.valueOf(maxValueInMap2);
                stringArray[3] = "HighClass" + ": " + String.valueOf(maxValueInMap4);
                ...
                BufferedReader br = new BufferedReader(new FileReader(PathToFirstClass));
                BufferedReader br2 = new BufferedReader(new FileReader(PathToHighClass));
                String line = null;
                ...
                //write cache
                File myObj = new File(TopVehiclePMCache);

                if (myObj.createNewFile()){
                    FileWriter myWriter = new FileWriter(TopVehiclePMCache);
                    myWriter.write("Time, Vehicle Type, Count\n"); //add time

                    for (Map.Entry<String, Integer> entry : firstClass1.entrySet()) {                                       //0-6PM
                        myWriter.write(entry.getKey() + "PM" + ", " + " FirstClass" + ", " + entry.getValue() + "\n");
                    }
                    for (Map.Entry<String, Integer> entry : highClass1.entrySet()) {
                        myWriter.write(entry.getKey() + "PM" + ", " + " HighClass" + ", " + entry.getValue() + "\n");
                    }
